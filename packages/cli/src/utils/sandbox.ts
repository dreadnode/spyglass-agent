...
import { SandboxConfig } from '@dreadnode/spyglass-agent-core';
...
export async function start_sandbox(
  ...
  // determine full path for spyglass-agent to distinguish linked vs installed setting
  ...
  // if BUILD_SANDBOX is set, then call scripts/build_sandbox.js under spyglass-agent repo
  //
  // note this can only be done with binary linked from spyglass-agent repo
  if (process.env.BUILD_SANDBOX) {
    if (!gcPath.includes('spyglass-agent/packages/')) {
      console.error(
        'ERROR: cannot build sandbox using installed gemini binary; ' +
          'run `npm link ./packages/cli` under spyglass-agent repo to switch to linked binary.',
      );
      process.exit(1);
    } else {
  ...
  if (!(await ensureSandboxImageIsPresent(config.command, image))) {
    const remedy =
      image === LOCAL_DEV_SANDBOX_IMAGE_NAME
        ? 'Try running `npm run build:all` or `npm run build:sandbox` under the spyglass-agent repo to build it locally, or check the image name and your network connection.'
        : 'Please check the image name, your network connection, or notify spyglass-agent-dev@google.com if the issue persists.';
    console.error(
